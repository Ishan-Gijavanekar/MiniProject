...
export const useAuthStore = create((set, get) => ({
    ...
    checkAuth: async() => {
      set({isLoading: true})
      try {
        const res = await axiosInstance.get(`/api/v1/users/get-user`)
    ...
    updateProfilePic: async(data) => {
      set({isLoading: true})
      try {
        const res = await axiosInstance.post(`/api/v1/users/update-profilePic`, data)
        set({userAuth: res.data.updatedUser})
    ...
    updateBackgroundPic: async(data) => {
      set({isLoading: true})
      try {
        const res = await axiosInstance.post(`/api/v1/users/update-backgroundPic`, data)
        set({userAuth: res.data.updatedUser})
